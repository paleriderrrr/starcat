...
**项目类型**: AI原生4X策略游戏 / Godot 4.6 客户端
> CURRENT_IMPLEMENTATION: GODOT. Rust/Wasm、React、Three.js、WebGL 与独立 Python 服务属于已退役的早期方案，不再作为当前开发目标。
...
#### 长期记忆 (结构化记忆与决策工件)
- 关键历史事件、关系快照与决策工件以结构化数据保存
- 当前运行时可写入 JSONL 决策账本；向量检索是后续研究扩展，不是已交付依赖
- 记忆节点结构:
  - 记忆ID (全局唯一)
  - 时间戳 (回合数)
  - 关联势力
...
使用 Godot Dictionary 与明确字段约束定义运行时协议:
- `GameLogic.gd` 是状态转换与规则校验的唯一权威
- `GameState.gd` 负责跨场景状态与信号分发
- `ApiClient.gd` 只提供本地服务门面与可选 LLM 调用，模型不能直接写入游戏状态
...
### 第一阶段: Godot 运行时 (Engine & Environment)
- [x] Godot 4.6 项目、AutoLoad 与主场景
- [x] 星系配置生成与 3D 星图
- [x] Godot Control HUD 与输入交互
- [x] 本地服务门面与可选 LLM Provider

### 第二阶段: 4X游戏循环 (Gameplay Loop)
- [x] 经济、研究、建造、造舰与殖民
- [x] 舰队移动、探索、战斗与外交
- [x] 回合状态机、AI 回合与多路径胜利判定

### 第三阶段: 植入灵魂 (AI Integration)
- [x] 结构化提示词、JSON 解析与本地回退
- [x] Godot 内建服务与可选 LLM Provider
- [x] 外交记忆、关系快照与 JSONL 决策工件
- [ ] 向量检索、离线 playbook 更新与长局实测

### 第四阶段: 社会与战争 (Diplomacy & Conflict)
- [x] AI 互信、条约、提案与自由外交消息
- [x] 导演事件、干预与叙事生成
- [x] 军事、外交与科技飞升胜利判定

### 第五阶段: UI交互与打磨 (Polish & UX)
- [x] 场景化 HUD 组件、视觉资产与星图动效
- [ ] 实机视觉 QA、长局可用性测试与性能优化
- [ ] 导出预设、CI 与发布流程
