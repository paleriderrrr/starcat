...
class GodotBackendMigrationTests(unittest.TestCase):
    ...
    def test_mimo_openai_compatible_provider_is_configurable_without_committing_key(self) -> None:
        config_source = _read_text("starcat/scripts/config/LocalConfig.gd")
        api_client_source = _read_text("starcat/scripts/autoload/ApiClient.gd")
        provider_source = _read_text("starcat/scripts/llm/OpenAIChatProvider.gd")
        smoke_source = _read_text("starcat/tools/mimo_provider_smoke_test.gd")
        gitignore_source = _read_text(".gitignore")

        self.assertIn('"provider": str(_project_setting("starcat/llm/provider", "bailian"))', config_source)
        self.assertIn('settings["provider"] = str(config.get_value(SECTION_LLM, "provider", settings["provider"]))', config_source)
        self.assertIn('OS.get_environment("MIMO_API_KEY")', config_source)
        self.assertIn('OS.get_environment("MIMO_MODEL")', config_source)
        self.assertIn('OS.get_environment("MIMO_BASE_URL")', config_source)
        self.assertIn('"mimo-v2.5-pro"', config_source)
        self.assertIn('"https://api.xiaomimimo.com/v1"', config_source)
        self.assertIn('if provider != "mimo":', config_source)
        self.assertIn('preload("res://scripts/llm/OpenAIChatProvider.gd")', api_client_source)
        self.assertIn('if str(_settings.get("provider", "bailian")).to_lower() == "mimo":', api_client_source)
        self.assertIn('const CHAT_COMPLETIONS_PATH: String = "/chat/completions"', provider_source)
        self.assertIn("const MIN_COMPLETION_TOKENS: int = 1024", provider_source)
        self.assertIn('"max_completion_tokens": maxi(max_output_tokens, MIN_COMPLETION_TOKENS)', provider_source)
        self.assertIn('"messages": [', provider_source)
        self.assertIn("func _auth_header() -> String:", provider_source)
        self.assertIn('str(_settings.get("auth_header", "")).to_lower() == "api-key"', provider_source)
        self.assertIn('"api-key: %s"', provider_source)
        self.assertIn('"Authorization: Bearer %s"', provider_source)
        self.assertNotIn('api_key.begins_with("tp-")', provider_source)
        self.assertIn('payload.get("choices", [])', provider_source)
        self.assertIn('OS.get_environment("MIMO_API_KEY")', smoke_source)
        self.assertIn('"mimo-v2.5-pro"', smoke_source)
        self.assertIn("Time.get_ticks_msec()", smoke_source)
        self.assertIn("elapsed_ms=%s", smoke_source)
        self.assertIn("JSON.parse_string(_extract_first_json_object(text))", smoke_source)
        self.assertIn("func _extract_first_json_object(text: String) -> String:", smoke_source)
        self.assertIn('str(parsed.get("provider", "")) != "mimo"', smoke_source)
        self.assertIn("invalid smoke response", smoke_source)
        self.assertIn("STARCAT_MIMO_PROVIDER_OK", smoke_source)
        self.assertIn("STARCAT_MIMO_PROVIDER_SKIPPED missing MIMO_API_KEY", smoke_source)
        self.assertNotIn("print(api_key", smoke_source)
        self.assertNotIn('settings["api_key"]', gitignore_source)
        self.assertIn('starcat/starcat.local.cfg', gitignore_source)

    def test_mimo_provider_does_not_reuse_bailian_credentials_by_default(self) -> None:
        config_source = _read_text("starcat/scripts/config/LocalConfig.gd")

        self.assertIn('"auth_header": str(_project_setting("starcat/llm/auth_header", ""))', config_source)
        self.assertIn('"mimo_api_key": str(_project_setting("starcat/llm/mimo_api_key", ""))', config_source)
        self.assertIn('settings["mimo_api_key"] = str(config.get_value(SECTION_LLM, "mimo_api_key", settings["mimo_api_key"]))', config_source)
        self.assertIn('settings["auth_header"] = str(config.get_value(SECTION_LLM, "auth_header", settings["auth_header"]))', config_source)
        self.assertIn('var mimo_api_key: String = str(settings.get("mimo_api_key", "")).strip_edges()', config_source)
        self.assertIn('if env_mimo_api_key != "":', config_source)
        self.assertIn('settings["api_key"] = env_mimo_api_key', config_source)
        self.assertIn('elif mimo_api_key != "":', config_source)
        self.assertIn('settings["api_key"] = mimo_api_key', config_source)
        self.assertLess(config_source.index('settings["api_key"] = env_mimo_api_key'), config_source.index('elif mimo_api_key != "":'))

    def test_star_map_visual_iteration_marks_reachable_routes_and_hidden_space(self) -> None:
        source = _read_text("starcat/scripts/StarMap.gd")

        self.assertIn("const REACHABLE_ROUTE_BEACON_RADIUS: float", source)
        self.assertIn("const FOG_OF_WAR_VEIL_RADIUS: float", source)
        self.assertIn("body.add_child(_make_reachable_route_beacon(system))", source)
        self.assertIn("body.add_child(_make_fog_of_war_veil(system))", source)
        self.assertIn('beacon.name = "ReachableRouteBeacon"', source)
        self.assertIn('veil.name = "FogOfWarVeil"', source)
        self.assertIn('beacon.set_meta("vfx_kind", "route_beacon")', source)
        self.assertIn('veil.set_meta("vfx_kind", "fog_veil")', source)
        self.assertIn("_animate_route_beacon(node, wave)", source)
        self.assertIn("_animate_fog_veil(node, wave)", source)
