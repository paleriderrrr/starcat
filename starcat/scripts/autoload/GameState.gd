...
var game_state: Dictionary = InitialData.create_initial_state()
...
func reset_state() -> void:
	game_state = InitialData.create_initial_state()
	selected_system_id = ""
	selected_fleet_id = ""
	ai_advice = ""
	world_data = {}
...
func get_player_faction() -> Dictionary:
	return GameLogic.player_faction(game_state)
...
func available_ship_types() -> Array:
	return GameLogic.available_ship_types(game_state)

func colonization_modes() -> Dictionary:
	return InitialData.colonization_modes()

func colonization_preview(system_id: String, mode: String) -> Dictionary:
	if selected_fleet_id == "":
		return {"allowed": false, "reason": "需要先选中一支己方舰队。", "cost": InitialData.colonization_modes().get(mode, {}).get("cost", {}), "turns": 0}
	return GameLogic.colonization_preview(game_state, selected_fleet_id, system_id, mode)
...
func get_visible_diplomatic_messages() -> Array:
	return GameLogic.visible_diplomatic_messages_for_player(game_state)
...
func get_active_narrative_events() -> Array:
	return GameLogic.active_narrative_events_for_player(game_state)
...
func get_relation_history(faction_id: String) -> Array:
	return GameLogic.relation_history_for_pair(game_state, PLAYER_FACTION_ID, faction_id)

func get_recent_intelligence_feed() -> Array:
	return GameLogic.recent_intelligence_feed(game_state, 10)

func get_strategic_posture_report() -> Dictionary:
	return GameLogic.strategic_posture_report(game_state, PLAYER_FACTION_ID)

func get_interception_report() -> Dictionary:
	return GameLogic.interception_report(game_state)
...
func start_research(tech_id: String) -> void:
	game_state = GameLogic.start_research(game_state, tech_id)
	state_changed.emit(game_state)

func cancel_research() -> void:
	game_state = GameLogic.cancel_research(game_state)
	state_changed.emit(game_state)

func queue_structure(system_id: String, building_type: String) -> void:
	game_state = GameLogic.queue_structure(game_state, system_id, building_type)
	state_changed.emit(game_state)
...
func trade_with_faction(target_faction_id: String) -> void:
	game_state = GameLogic.trade_with_faction(game_state, target_faction_id)
	state_changed.emit(game_state)

func threaten_faction(target_faction_id: String) -> void:
	game_state = GameLogic.threaten_faction(game_state, target_faction_id)
	state_changed.emit(game_state)

func propose_treaty(target_faction_id: String, treaty_type: String) -> void:
	game_state = GameLogic.propose_treaty(game_state, target_faction_id, treaty_type)
	state_changed.emit(game_state)

func revoke_treaty(target_faction_id: String, treaty_type: String) -> void:
	game_state = GameLogic.revoke_treaty(game_state, target_faction_id, treaty_type)
	state_changed.emit(game_state)
...
func explore_system(system_id: String) -> void:
	if selected_fleet_id == "":
		return
	game_state = GameLogic.explore_system(game_state, selected_fleet_id, system_id)
	state_changed.emit(game_state)

func colonize_system(system_id: String, mode: String = "STANDARD") -> void:
	if selected_fleet_id == "":
		return
	game_state = GameLogic.colonize_system(game_state, selected_fleet_id, system_id, mode)
	state_changed.emit(game_state)

func move_selected_fleet(system_id: String) -> void:
	if selected_fleet_id == "":
		return
	game_state = GameLogic.move_fleet(game_state, selected_fleet_id, system_id)
	selected_system_id = system_id
	state_changed.emit(game_state)
	selection_changed.emit(selected_system_id, selected_fleet_id)
...
func advance_turn() -> void:
	...
	if has_node("/root/ApiClient") and backend_status == "online":
		...
		game_state = GameLogic.process_turn(game_state)
		turn_busy = false
		state_changed.emit(game_state)
...
func apply_director_event_to_world(event_template_id: String) -> void:
	...
	if target_system_id == "":
		return
	game_state = GameLogic.trigger_narrative_event(game_state, event_template_id, target_system_id, [PLAYER_FACTION_ID])
	state_changed.emit(game_state)

func apply_director_intervention_to_world(intervention_type: String) -> void:
	game_state = GameLogic.apply_director_intervention(game_state, intervention_type, 0.6, 4)
	state_changed.emit(game_state)

func resolve_narrative_event(event_id: String, option_label: String) -> void:
	game_state = GameLogic.resolve_narrative_event_choice(game_state, event_id, option_label)
	state_changed.emit(game_state)
...
func request_diplomatic_message(faction_id: String, tone: String) -> void:
	if has_node("/root/ApiClient"):
		...
		if target.is_empty():
			return
		var relation: Dictionary = GameLogic.relation_between(game_state, PLAYER_FACTION_ID, faction_id)
		ApiClient.request_diplomatic_message(get_player_faction(), target, relation.get("level", "NEUTRAL"), tone)
...
func send_player_message(target_faction_id: String) -> void:
	...
	game_state = GameLogic.player_freeform_message(game_state, target_faction_id, message_text, visibility)
	...
	diplomacy_changed.emit()
	if has_node("/root/ApiClient"):
		var relation: Dictionary = GameLogic.relation_between(game_state, PLAYER_FACTION_ID, target_faction_id)
		var intent_preview: Dictionary = GameLogic.describe_player_diplomatic_intent(message_text)
		ApiClient.request_ai_conversation(
			...
			message_text,
			visibility,
			GameLogic.parse_player_diplomatic_intent(message_text).get("type", "MESSAGE"),
			intent_preview.get("detail", "")
		)

func accept_diplomatic_proposal(proposal_id: String) -> void:
	game_state = GameLogic.accept_pending_proposal(game_state, proposal_id)
	state_changed.emit(game_state)
	diplomacy_changed.emit()

func reject_diplomatic_proposal(proposal_id: String) -> void:
	game_state = GameLogic.reject_pending_proposal(game_state, proposal_id)
	state_changed.emit(game_state)
	diplomacy_changed.emit()
...
func _on_conversation_received(payload: Dictionary) -> void:
	if _pending_conversation_target_id != "":
		game_state = GameLogic.receive_ai_reply(game_state, _pending_conversation_target_id, payload.get("title", ""), payload.get("content", ""), _pending_conversation_visibility, payload.get("tone", "neutral"))
		state_changed.emit(game_state)
		diplomacy_changed.emit()
	diplomatic_message = payload
	ai_advice = payload.get("structured_text", ai_advice)
...
func _on_merchant_decision_received(payload: Dictionary) -> void:
	game_state = GameLogic.process_turn(game_state, payload)
	ai_advice = payload.get("structured_text", ai_advice)
	turn_busy = false
	state_changed.emit(game_state)
	advisor_changed.emit(ai_advice, world_data, diplomatic_message)
